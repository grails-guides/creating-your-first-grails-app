...
Now (still editing `VehicleController.groovy`), modify the `index` action as seen below:
...
modify the `show` action as seen below:
...
.grails-app/controllers/org/grails/guides/VehicleController.groovy
----
include::{sourceDir}/grails-app/controllers/org/grails/guides/VehicleController.groovy[]
----
